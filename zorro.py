...
# from fidelity import fidelity as distortion
...
class SISDistortionGraphExplainer(AbstractGraphExplainer):
    ...
    def argmax_distortion_general(self,
        ...
        if self.greedy:
            ...
            if initialization:
                ...
                # # only in case of both nodes and features zero?
                # if self.ensure_improvement and best_distortion_improve < 1 / self.distortion_samples - .00000001:
                #     best_element, best_distortion_improve, raw_sorted_elements = self.argmax_distortion_general_full(
                #         previous_distortion,
                #         possible_elements,
                #         selected_elements,
    ...
    def _determine_minimal_set(self, initial_distortion, tau, possible_nodes, possible_features,
        ...
        while current_distortion <= 1 - tau:
            ...
            if self.ensure_improvement and \
                ...
                # TODO restrict recalculations to cases where something changed
                # recalculate features only if node was added (and vise versa)
                best_node, improve_in_distortion_by_node = self.argmax_distortion_general(
                    current_distortion,
                    possible_nodes,
                    self.selected_nodes,
                ...
                # # TODO option to stop in greedy variant?
                # if improve_in_distortion_by_node < .00000001 and improve_in_distortion_by_feature < .00000001:
                #     best_node, improve_in_distortion_by_node = self.argmax_distortion_general(
                #         current_distortion,
                #         possible_nodes,
                #         self.selected_nodes,
    ...
    def recursively_get_minimal_sets(self, initial_distortion, tau, possible_nodes, possible_features,
        ...
        # TODO check if these two lines can be moved to _determine_minimal_set
...
class GradientBasedDistortionGraphExplainer(AbstractGraphExplainer):
    coeffs = {
        'fidelity': 1,
        'node_size': 0.01,
        'node_ent': 0.1,
    ...
    def __init__(self, model, device, log=True, record_process_time=False, samples=100, learning_rate=0.01):
        super(GradientBasedDistortionGraphExplainer, self).__init__(
            model=model,
            device=device,
            log=log,
            record_process_time=record_process_time,
...
def load_minimal_nodes_and_features_sets(path_prefix, node, check_for_initial_improves=False):
    ...
    # TODO check how to turn the last of
...
def comp2D_vect(a1, a2):
    ...
    midx = np.ravel_multi_index(a2.T, a2.max(0) + 1)
    # print("midx", midx)
    a, idx, inv, cnt = np.unique(midx, return_index=True, return_inverse=True, return_counts=True, )
    # print("a", a)
    # print("idx", idx)
    # print("inv", inv)
    # print("cnt", cnt)
    ...
    # print((inv[:,None]==m2))
    _, relevant_idx = (inv == m2[:, None]).nonzero()
    # print("relevant_idx", relevant_idx)
    return out, relevant_idx, np.cumsum(out) - 1


def distortion(model, node_idx=None, full_feature_matrix=None, computation_graph_feature_matrix=None,
               edge_index=None, node_mask=None, feature_mask=None, predicted_label=None, samples=None,
               random_seed=12345, device="cpu", conditional_samples=False, validity=False,
    ...
    if conditional_samples:
        restricted_computational_graph_features = computation_graph_feature_matrix[
                                                  torch.squeeze(node_mask) > 0, :][
                                                  :, torch.squeeze(feature_mask.T) > 0].numpy()

        restricted_complete_features = full_feature_matrix[:, torch.squeeze(feature_mask.T) > 0].numpy()

        # !!! requires INT values only in the feature matrix
        index_max, index, index_end = comp2D_vect(restricted_computational_graph_features.astype(np.int),
                                                  restricted_complete_features.astype(np.int))

        raw_rand = np.random.rand(samples, torch.squeeze(node_mask).numpy().sum().astype(np.int))

        raw_rand = raw_rand * index_max
        raw_rand = np.floor(raw_rand).astype(np.int)
        raw_ind = index[index_end.T - raw_rand].astype(np.int)
        random_indices[:, torch.squeeze(node_mask) > 0, :] = torch.tensor(
            raw_ind[:, :, None].repeat(num_features, axis=-1),
            dtype=torch.int64,
            device=device)
...
def multi_node_precompute_full_distortion(model,
    ...
    num_hops = SISDistortionGraphExplainer.num_hops(model)
    flow = SISDistortionGraphExplainer.flow(model)
